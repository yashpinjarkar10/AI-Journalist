...
def main(): 
    ...
    with st.sidebar:
        st.header("Settings")
        # source_type varialble will store the option that user select
    ...
    with col2:
        add_disabled = len(st.session_state.topics) >= 4 or not new_topic.strip()
            ...
            with st.spinner("🔍 Analyzing topics and generating audio..."):
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        st.text_area("📰 News Info", news_summary, height=200)
                        st.download_button(
